...
public class SpuAttrValueController {
    ...
    @ApiOperation("根据spuId和cid查询attrValue列表")
    public ResponseVo<List<SpuAttrValueEntity>> querySearchSpuValueByCidAndSpuId(
            @RequestParam("cid") Long cid,@PathVariable("spuId") Long spuId
    ){
        List<SpuAttrValueEntity> spuAttrValueEntities= spuAttrValueService.querySearchSpuValueByCidAndSpuId(cid,spuId);
